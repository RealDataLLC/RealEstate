...
def linear_trend(data):
    ...
    """
    result = pd.DataFrame(np.nan, index=np.arange(STARTING_YEAR, FINAL_YEAR), columns=['x', 'constant', 'adoption'],

                          dtype=np.float64)
    result.index.name = 'Year'
    if data.dropna().empty: return result
    y = data.dropna().values
    x = data.dropna().index - STARTING_YEAR
...
def poly_degree2_trend(data):
    """2nd degree polynomial trend model.
       ...
       Arguments: data is a pd.Series used to provide the x+y for curve fitting.
    """
    result = pd.DataFrame(np.nan, index=np.arange(STARTING_YEAR, FINAL_YEAR),
...
def single_trend(data):
    ...
    """
    result = pd.DataFrame(0, index=np.arange(STARTING_YEAR, FINAL_YEAR),
